...
def _connection_id(model_cfg: dict[str, Any]) -> str | None:
    return str(model_cfg.get("connection_id") or "").strip() or None
    ...
    *,
    ...
    for kind in ("stt_config", "tts_config", "llm_config"):
        ...
        if _requires_stored_auth(provider):
            ...
            cache_key = f"{provider}:{connection_id}" if connection_id else provider
            if cache_key not in cache:
                cache[cache_key] = await client.get_provider_auth(
                    provider, org_id, connection_id=connection_id
                )
            merged[kind] = {**blob, **cache[cache_key]}
            logger.info(
                "Merged auth into {} provider={} keys={}",
                kind,
                provider,
                sorted(cache[cache_key].keys()),
            )
        else:
            merged[kind] = dict(blob)
            logger.info("No stored auth needed for {} provider={}", kind, provider)
    return merged


# Fields a language switch re-applies through a settings delta (settings.py).
_DELTA_FIELDS = frozenset({"model", "temperature", "max_tokens", "top_p", "top_k"})


def _pool_key(kind: ServiceKind, config: dict[str, Any]) -> tuple[Any, ...]:
    key: tuple[Any, ...] = (
        kind,
        _provider_id(config),
        str(config.get("model") or "").strip(),
    )
    connection_id = _connection_id(config)
    if connection_id:
        # Two connections may serve the same model id from different endpoints,
        # and whatever a switch cannot re-apply (history / system-prompt mode,
        # penalties, endpoint auth) is fixed when the service is built — so
        # languages share a connection-based service only when all of it matches.
        fixed = sorted(
            (name, repr(value))
            for name, value in config.items()
            if name not in _DELTA_FIELDS
        )
        key += (connection_id, tuple(fixed))
    return key
...
async def build_language_switchers(
    ...
    *,
    ...
    # Flat legacy models without language.primary resolve to one stack keyed by
    # the STT language; fall back to that rather than failing.
    langs = configured_languages(agent) or list(raw_stacks)
    primary = langs[0]
