...
class _LanguageRoutingMixin:
    ...
    async def _report_service_failure(self, failed_service: Any, error: ErrorFrame) -> None:
        """Pass the active service's own error on instead of re-attributing it.

        ``ServiceSwitcher`` reports it as the switcher's, categorised unknown,
        and stays usable while any member is. Languages never fail over, so a
        dead active service leaves the call unable to go on in its language:
        the worker's unusable policy and the error handlers must judge it by
        that service, name and category intact, as they did before switching.
        """
        # Skip ServiceSwitcher.push_frame: it would route the error back here.
        await super(ServiceSwitcher, self).push_frame(error, FrameDirection.UPSTREAM)  # type: ignore[misc]
