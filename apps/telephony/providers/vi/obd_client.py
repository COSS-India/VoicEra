"""VI CPaaS OBD outbound API client (env credentials)."""
...
def vi_env_credentials_configured() -> bool:
    """True when OBD username and password are present in the environment."""
    return bool(
        (os.environ.get("VI_OBD_USERNAME") or "").strip()
        and (os.environ.get("VI_OBD_PASSWORD") or "").strip()
    )
...
class ViObdClient:
    ...
    def create_campaign(
        ...
        *,
        ...
        """Create an OBD campaign with a near-immediate time window."""
    ...
    def get_active_dni(self, token: str, flow_id: str) -> dict:
        """Fetch active DNI list for a flow."""
