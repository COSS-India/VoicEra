...
If you just finished the [agent creation wizard](create-an-agent.md), a test call is ready on the last step — click **Start test call**. To test an existing agent later, find its card on the dashboard's home screen and click **Test on Browser**.
...
<Note>
Only agents set up for **WebSocket** delivery (see [the Delivery step of the wizard](create-an-agent.md#delivery-how-callers-reach-it)) can be tested this way. An agent set up with a phone number instead will offer to place a real outbound call.
</Note>

<Tip>
Test calls are still recorded — you'll find the transcript later in [call history](everyday-tasks.md#check-call-history) just like a real call.
</Tip>
...
<Note>
You need a telephony provider (like Plivo) connected under [Integrations](sign-up.md#2-connect-your-ai-services) before a provider shows up here.
</Note>
...
| No telephony provider shows up when adding a number | Connect one under [Integrations](sign-up.md#2-connect-your-ai-services) first. |

For deeper audio or call-connection problems, see [Voice and audio problems](../troubleshooting/voice-and-audio.md).

## Next

[Run a calling campaign](run-a-campaign.md) — call a whole list of people at once.
