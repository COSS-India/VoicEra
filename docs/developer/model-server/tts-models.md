...
**Measured, and it does not reach realtime.** On ace-h200 GPU 1, 15 Sep 2026: ~45 tok/s warm, RTF 2.05–2.33, TTFA ~370 ms, against the 100 tok/s realtime needs — so about 2.2× too slow to hold a live call. Audio starts promptly and then underruns. A cold context runs the first generation at 12.5 tok/s, which is what the 256-token warmup absorbs. The cost is upstream's loop, not the slot: calling `generate_audio` directly with nothing in the way reproduces the same rate to the decimal. Closing the gap means an inference engine rather than tuning — vLLM already supports Cohere2, so an adapter may be tractable. Treat it as demo and evaluation grade, and note `RUMIK_MAX_CONCURRENCY` is 1 because one stream already underruns.
