...
| `local` | `apps/providers/local/` | Talks to VoicEra's own [model server](../model-server/overview) gateway instead of a third-party API. Three providers today: `indic_orpheus` (TTS), `rumik_oss` (TTS) and `indic_nemotron` (STT). |
...
**22 cloud vendors, 2 adapters, and 3 local providers.** Several vendors register for more than one kind, so the per-kind lists overlap.
...
| `rime` | Rime | cloud | TTS |
| `rumik_oss` | Rumik OSS | **local** | TTS |
| `sarvam` | Sarvam | cloud | STT, TTS, LLM |
| `smallest` | Smallest.ai | cloud | STT, TTS |
| `speechmatics` | Speechmatics | cloud | STT |
| `xai` | xAI | cloud | TTS |

That is 13 STT providers, 16 TTS providers, and 10 LLM providers. No local LLM provider exists yet — see [Adding an AI provider](../guides/adding-a-provider#local-providers-one-extra-registration-call).
