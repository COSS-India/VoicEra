...
| The runtime reachable at `NEXT_PUBLIC_RUNTIME_WS_URL` | `ws://localhost:7860` running from source; `wss://vobiz.johnaic.com` in the Compose default. Unlike the REST API, this URL is not proxied — the browser must reach it directly. See [Running the dashboard](running.md). |
...
sequenceDiagram
  ...
  participant W as "AudioWorklet<br/>stream-processor · playback"
  ...
  loop "while connected"
    M->>B: "Float32 audio<br/>ScriptProcessorNode, 512-sample blocks"
...
`org_id` comes from the stored session, `agent_id` from the agent being tested. Before opening the socket the hook calls `createWebCall({ agent_id })` — an authenticated `POST /calls/web` — so the CallLog exists and the browser knows the `call_id` up front. The socket itself carries no handshake message and no auth token: the runtime resolves the agent from the path and loads its config and provider credentials from the API itself.
