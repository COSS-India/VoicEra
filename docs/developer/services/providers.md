...
The inventory is **22 cloud vendors, 2 adapters, and 3 local providers**. The adapters are `bhashini` (STT and TTS) and `kenpath` (LLM only); the local providers are `indic_orpheus` (TTS), `rumik_oss` (TTS) and `indic_nemotron` (STT) — so no single kind lists all 27. This table was generated by calling `provider_schemas()` on the checked-out tree:
...
| TTS | 16 | `azure_speech`, **`bhashini`** (adapter), `camb`, `cartesia`, `deepgram`, `elevenlabs`, `google`, **`indic_orpheus`** (local), `inworld`, `lmnt`, `openai`, `rime`, **`rumik_oss`** (local), `sarvam`, `smallest`, `xai` |
| LLM | 10 | `atlascloud`, `aws_bedrock`, `azure_openai`, `google`, `google_vertex`, `groq`, **`kenpath`** (adapter), `openai`, `openrouter`, `sarvam` |

Many vendors appear under more than one kind — `openai`, `google`, `sarvam`, `deepgram`, `cartesia`, `elevenlabs`, `azure_speech`, and `smallest` each register two or three. Credentials are stored once per vendor in `ProviderAuth` and shared across its slots. `indic_orpheus`, `rumik_oss` and `indic_nemotron` have no stored credentials at all — see [The local providers](#the-local-providers).
...
Deferring the import means the registration decorator runs with no vendor SDK present. The API can therefore enumerate all 27 providers, validate agent configs against them, and serve `GET /configuration/*` while installing none of them. Only the runtime, and only at the moment it creates a service for a call, needs the extra on disk.
...
`local/` holds providers that talk to VoicEra's own [model server](../model-server/overview) gateway instead of a third-party API. Three ship today.
...
`local/rumik_oss/` wraps the same OpenAI-compatible `/v1/audio/speech` endpoint as Indic Orpheus, on the same 24 kHz signed-16-bit wire, with the same no-`Auth` arrangement — but it models the opposite of the **voice implies language** rule. Rumik OSS-1 has four voices (Ira, Aisha, Siya, Zoya) and every one of them covers every language, so `voice` and `language` are independent and the roster is the same four names under each of the 22 canonical ids the catalog declares.

`style` is the other difference. Orpheus matches `instructions` against a fixed roster and ignores anything that does not match; Rumik conditions on free text, so the catalog marks `style` as `input_type: "both"` with `allow_custom_input` — the options are a starting point, in the model's own `emotion, accent, pace` shape, not a vocabulary. A typo therefore changes how a line is read rather than being silently dropped.

The language map is 21 Indic languages plus English, not the 22 the model card claims. The card never enumerates them; what Rumik publishes is WER and CER for 21, which are the Eighth Schedule languages with only Sindhi absent. Adding the missing one on the strength of an inference would mean agents configured for a language nobody has tested, so `catalog.py` leaves it out and records why — see [TTS models → rumik-oss-1](../model-server/tts-models#rumik-oss-1) for the licence, which is non-commercial.
...
`gateway_model_id` is the model server's own slot id (its `models.yaml`/folder name under `model-server/<kind>/`), a separate namespace from the `apps/providers` provider id — `indic_orpheus` → gateway id `"orpheus"`, `rumik_oss` → gateway id `"rumik-oss-1"`, `indic_nemotron` → gateway id `"indic-nemotron"`.
